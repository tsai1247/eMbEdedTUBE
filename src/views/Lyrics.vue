...
onMounted(() => {
  if(route.query.lyrics) {
    playing.value = lyrics.find(
      (lyric) => lyric.name == route.query.lyrics
    );
  }
})

watch(() => playing.value, () => {
  console.log(playing.value.name)
  if(playing.value.name !== route.query.lyrics) {
    console.log('change');
    router.replace({query: {...route.query, lyrics: playing.value.name} });
  }
}, {depp: true})
